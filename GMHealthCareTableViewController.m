//
//  GMHealthCareTableViewController.m
//  DogFood
//
...
@interface GMHealthCareTableViewController ()

@end

@implementation GMHealthCareTableViewController
{
    NSArray *healthCares;
...
@synthesize delegate, healthCare, firstLevel;
...
- (void)viewDidLoad
{
    ...
    healthCares = [firstLevel objectForKey:@"HealthCare"];    

    selectedIndex = [healthCares indexOfObject:self.healthCare];      
}
...
{
    static NSString *CellIdentifier = @"HealthCareCell";
    ...
    // Add text to cell
    cell.textLabel.text = [healthCares objectAtIndex:indexPath.row];
    cell.detailTextLabel.text = @"";
    
    // Checkmark health care selected    
    ...
    if ((medicinePressed || breedPressed) || ([selectOne isEqualToString:@"Select One"])) 
    {
        cell.accessoryType = UITableViewCellAccessoryNone;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	cell.accessoryType = UITableViewCellAccessoryCheckmark;
	NSString *theHealthCare = [healthCares objectAtIndex:indexPath.row];
	[self.delegate gMHealthCareTableViewController:self didSelectHealthCare:theHealthCare];
}
...
- (void) customView
{
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(100, 20, 180, 24)];
