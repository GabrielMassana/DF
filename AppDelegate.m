...
@property (nonatomic, strong) GMDogFoodDataBase *dataBase;
@end
...
-(GMDogFoodDataBase *) dataBase
{
    if (!_dataBase)
    {
        _dataBase = [[GMDogFoodDataBase alloc] init];
    }
    return _dataBase;
}
    ...
    if ([[self.fetchedResultsController fetchedObjects] count] == ??? ) 
     {
        NSLog(@"WOW we've got ???!!!!");        
        // Delete Data
        [self.dataBase deleteAllDataFromDataBase];        
        NSLog(@"!!!!! ~~> WOW BBDD data deleted!");        
    }
    */
    
    
    // FOR UPLOAD DATA TO DATABASE
    // If the DataBase is empty, call importCoreDataDefaultFoodProducts to import DataBase
    // Else skip
    [self setupFetchedResultsController];
    ...
    if (![[self.fetchedResultsController fetchedObjects] count] > 0 )
    {
        ...
        //Import Data
        [self.dataBase importCoreDataDefaultFoodProducts];
    }
    else
    {
...
// create request and perform fetch
...
{    
    ...
    //request.predicate = [NSPredicate predicateWithFormat:@"FoodProducts = Blah"];
